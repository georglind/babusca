...
def g2s_num_00(d, phi, U, offsets):
    offset = 1e3   # offset
    omegas = [0] * 2
    Us = [U, U]
    se = ql.QL(2, omegas, Us, xs=[0, phi / (offset)])

    g2s = np.zeros(offsets.shape, dtype=np.float64)
    for i, off in enumerate(offsets):
        se.reset()
        se.model.es = [offset - off - d, offset - off + d]
        res = g2.coherent_state_tau0(se, (0, 0), (0, 0), 2 * offset)
        g2s[i] = res['g2'][0]
